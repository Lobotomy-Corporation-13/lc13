...
/mob/living/simple_animal/hostile/megafauna/bubblegum/proc/bloodsmack(turf/T, handedness)
	...
	for(var/mob/living/L in T)
		if(!faction_check_mob(L))
			...
			L.deal_damage(10, BRUTE, src, forced = TRUE, overrides = list(def_zone = limb_to_hit, blocked = L.run_armor_check(limb_to_hit, MELEE, null, null, armour_penetration), wound_bonus = CANT_WOUND))
	SLEEP_CHECK_DEATH(3)
...
/mob/living/simple_animal/hostile/megafauna/bubblegum/Bump(atom/A)
	if(charging)
		...
		if(isliving(A))
			...
			L.deal_damage(istype(src, /mob/living/simple_animal/hostile/megafauna/bubblegum/hallucination) ? 15 : 30, BRUTE, src, overrides = list(wound_bonus = CANT_WOUND, spread_damage = TRUE))
