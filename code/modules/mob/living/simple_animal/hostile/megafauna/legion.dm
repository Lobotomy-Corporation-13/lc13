...
/mob/living/simple_animal/hostile/megafauna/legion/throw_impact(mob/living/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(istype(hit_atom))
		...
		hit_atom.deal_damage(22 * size / 2, BRUTE, src, overrides = list(wound_bonus = CANT_WOUND, spread_damage = TRUE)) //It gets pretty hard to dodge the skulls when there are a lot of them. Scales down with size
