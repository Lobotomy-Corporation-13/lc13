...
/mob/living/simple_animal/hostile/ordeal/violet_fruit/Life()
	...
	for(var/mob/living/L in view(2, src))
		if(!faction_check_mob(L))
			...
			L.deal_damage(5, BLACK_DAMAGE, src, forced = TRUE)
	return TRUE
...
/mob/living/simple_animal/hostile/ordeal/violet_fruit/proc/ReleaseDeathGas()
	...
	for(var/mob/living/L in livinginrange(15, target_c))
		if(faction_check_mob(L))
			continue
		L.deal_damage(33, WHITE_DAMAGE, forced = TRUE)
...
/mob/living/simple_animal/hostile/ordeal/violet_monolith/Life()
	...
	for(var/mob/living/L in view(2, src))
		if(!faction_check_mob(L))
			new /obj/effect/temp_visual/revenant(get_turf(L))
			L.deal_damage(9, BLACK_DAMAGE, src, forced = TRUE)
...
/mob/living/simple_animal/hostile/ordeal/violet_monolith/proc/FallDown()
	...
	for(var/mob/living/L in view(4, src))
		if(!faction_check_mob(L))
			...
			L.deal_damage((150 - distance_decrease), RED_DAMAGE, src)
			if(L.health < 0)
				L.gib()
	SLEEP_CHECK_DEATH(5)
		...
		for(var/turf/TT in RANGE_TURFS(3, T))
			...
			for(var/mob/living/L in TT)
				...
				if(!CanAttack(L))
					continue
				been_hit += L
				L.deal_damage(attack_damage, RED_DAMAGE, src)
		...
		for(var/turf/TT in RANGE_TURFS(3, T))
			...
			for(var/mob/living/L in TT)
				...
				if(!CanAttack(L))
					continue
				been_hit += L
				L.deal_damage(attack_damage, WHITE_DAMAGE, src)
		...
		for(var/turf/TT in RANGE_TURFS(2, T))
			...
			for(var/mob/living/L in TT)
				...
				if(!CanAttack(L))
					continue
				been_hit += L
				L.deal_damage(attack_damage, BLACK_DAMAGE, src)
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/pale/proc/Pulsate()
	...
	for(var/turf/T in RANGE_TURFS(pulse_range, eye))
		...
		for(var/mob/living/L in T)
			if(!CanAttack(L))
				continue
			L.deal_damage(pulse_damage, PALE_DAMAGE, src)
			has_targets = TRUE
