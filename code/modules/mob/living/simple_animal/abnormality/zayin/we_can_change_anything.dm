...
/mob/living/simple_animal/hostile/abnormality/we_can_change_anything/Worktick(mob/living/carbon/human/user, bubble_type = ABNO_BALLOON_GENERIC | ABNO_BALLOON_SPECIFIC, work_type)
	if(!sacrifice)
		user.deal_damage(5, RED_DAMAGE, forced = TRUE) // say goodbye to your kneecaps chucklenuts!
	else
		...
		switch(ramping_speed)
			...
			if(8 to 20)
				ramping_speed -= 0.5
		user.deal_damage(8, RED_DAMAGE, forced = TRUE) // say goodbye to a bit more than your kneecaps... (total damage is 800 RED).
		total_damage += 8
	return ..()
...
/mob/living/simple_animal/hostile/abnormality/we_can_change_anything/proc/Grind()
	...
	while(grind_end > world.time)
		...
		for(var/mob/living/carbon/human/victim in get_turf(src))
			victim.deal_damage(grind_damage, RED_DAMAGE, src, forced = TRUE)
			if(victim.health <= 0)
				victim.gib()
		stoplag(1)
	unbuckle_all_mobs()
