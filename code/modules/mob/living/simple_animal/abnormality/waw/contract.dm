...
/obj/effect/proc_holder/spell/pointed/contract/recall/Click()
	if(is_pulling)
		to_chat(owner, span_warning("You are currently pulling someone over!"))
		return
	if (contracted != null)
		// do recall
		is_pulling = TRUE
...
/datum/action/cooldown/contracted_passage/proc/EndTeleport(mob/living/simple_animal/hostile/abnormality/contract/C)
	...
	if(current_turf && current_turf.density && starting_location)
		C.forceMove(starting_location)
		to_chat(C, span_warning("You were trapped in a wall and have been returned to your starting location!"))
