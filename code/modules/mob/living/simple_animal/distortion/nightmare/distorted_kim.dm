...
/mob/living/simple_animal/hostile/distortion/kim/proc/Finisher(mob/living/target) //This is super hard to avoid
	target.deal_damage(10, PALE_DAMAGE, src, overrides = list(blocked = target.run_armor_check(null, RED_DAMAGE), spread_damage = TRUE)) //10% of your health in red damage
	to_chat(target, span_danger("[src] is trying to cut you in half!"))
	if(!ishuman(target))
		target.deal_damage(100, PALE_DAMAGE, src) //bit more than usual DPS in pale damage
		return
	if(target.health > 0)
		return
	var/mob/living/carbon/human/H = target
