...
/obj/item/apprentice_recruitment/thumb_nursefather/recruit_apprentice(mob/living/carbon/human/H, mob/living/user)
	// Check for Ring system — warn and strip if present
	var/datum/component/artistic_exp/ring_exp = H.GetComponent(/datum/component/artistic_exp)
	var/datum/component/corporist_student/ring_student = H.GetComponent(/datum/component/corporist_student)
	if(ring_exp || ring_student)
		var/confirm = tgui_alert(user, "[H] is part of the Ring's artistic system. Recruiting them will permanently remove their Ring progress. Continue?", "Ring Conflict", list("Continue", "Cancel"))
		if(confirm != "Continue")
			return
		// Strip the Ring system completely
		// Remove all ring skill components
		for(var/datum/component/ring_skill/skill in H.GetComponents(/datum/component/ring_skill))
			qdel(skill)
		// Remove corporist student component (handles trait + actions)
		if(ring_student)
			qdel(ring_student)
		// Remove inspired artist component
		var/datum/component/inspired_artist/insp = H.GetComponent(/datum/component/inspired_artist)
		if(insp)
			qdel(insp)
		// Remove artistic EXP
		if(ring_exp)
			ring_exp.refund_all_points()
			qdel(ring_exp)
		// Remove Ring antagonist datum
		if(H.mind)
			var/datum/antagonist/ring_artist/ring_antag = H.mind.has_antag_datum(/datum/antagonist/ring_artist)
			if(ring_antag)
				H.mind.remove_antag_datum(ring_antag.type)
		to_chat(H, span_boldwarning("Your connection to the Ring's artistic traditions has been severed."))
		to_chat(user, span_notice("[H]'s Ring affiliations have been removed."))
