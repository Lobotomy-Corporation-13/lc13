...
/obj/projectile
	...
	/// How many pixels we pass in a single SSprojectiles tick
	var/speed = 0.8
	/// Because the calculation will change everything and i dont want to change every number in this PR.
	var/true_speed = 0.8
...
/obj/projectile/Initialize(mapload)
	...
	//Ill fix this and change all speed values when i regain my strength. -IP
	true_speed = 1 / speed
	if(!nondirectional_sprite && isnum(angle))
...
/obj/projectile/proc/fire(fire_angle, atom/direct_target)
	...
	if(!nondirectional_sprite)
		...
		matrix.Turn(angle)
		transform = matrix
	movement_vector = new(true_speed, angle)
	if (hitscan)
		beam_points = list()
	...
	if (!deletion_queued && !hitscan)
		process_movement(max(FLOOR(true_speed, 1), 1), tile_limit = TRUE)
...
/obj/projectile/process()
	...
	var/pixels_to_move = elapsed_time * SSprojectiles.pixels_per_decisecond * true_speed + overrun
	overrun = 0
...
/obj/projectile/proc/preparePixelProjectile(atom/target, atom/source, list/modifiers = null, deviation = 0)
	...
	if(target_loc)
		var/final_angle = get_angle(src, target_loc)
		yo = target_loc.y - source_loc.y
		xo = target_loc.x - source_loc.x
		var/if_successful = set_angle(final_angle + deviation)
		stack_trace("Projectile [type] fired at [final_angle]. [if_successful]")
		return TRUE
...
/obj/projectile/proc/reflect(atom/hit_atom)
	...
	var/new_angle_s = WRAP(angle + rand(120, 240),0,360)
