...
/client/proc/adminchangesubmap()
	...
	set desc = "Set the next map's variant, if it has any."

	var/datum/map_config/map_config = SSmapping.next_map_config
	...
	for(var/submap in map_config.available_submaps)
		...
		if(istext(map_config.available_submaps[submap]))
			display_name = map_config.available_submaps[submap]
		else
		...
		if(VM.has_submaps && VM.available_submaps.len > 1)
			...
			switch(submap_choice)
				...
				if("Start Vote")
					...
					to_chat(world, span_boldannounce("An administrator is opening a vote for a selected map's variant. A vote will start shortly to choose which one to play!"))
				if("Random")
					VM.SetSelectedSubmap(SUBMAP_DEFAULT_RANDOM)
					message_admins("[key_name_admin(usr)] randomly selected [VM.available_submaps[VM.map_file]] variant for [VM.map_name]")
