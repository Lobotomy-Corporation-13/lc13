...
/datum/status_effect/stacking/lc_burn/proc/DealDamage()
	owner.deal_damage(stacks, FIRE, forced = TRUE)
...
/datum/status_effect/stacking/lc_overheat/tick()
	...
	if(ishuman(owner))
		...
		owner.deal_damage(stacks*4, FIRE, forced = TRUE) // x4 on non humans (Average burn stack is 20. 80/5 sec, extra 16 pure dps)
...
/datum/status_effect/stacking/pallid_noise/tick()//TODO:change this to golden apple's life tick for less lag
	if(!ishuman(owner))
		owner.deal_damage(stacks * 5, WHITE_DAMAGE, forced = TRUE)
		return
