              ...
              <Section title="Available Skills" fill scrollable>
                {available_skills.length === 0 ? (
                  <Box color="gray" textAlign="center" p={2}>
                    No skills available for this rank
                  </Box>
                ) : (
                  <Table compact>
                    <Table.Row header>
                      <Table.Cell>Skill</Table.Cell>
                      <Table.Cell textAlign="center" width="10%">Level</Table.Cell>
                      <Table.Cell textAlign="center" width="10%">Slots</Table.Cell>
        ...
        <Stack.Item>
          <Stack>
            ...
            <Stack.Item grow>
              <Button
                fluid
                content={busy ? 'Fabricating...' : 'Fabricate Augment'}
                color="good"
...
const MaterialsDisplay = (props, context) => {
  const { act, data } = useBackend(context);
  const { stored_materials = [] } = data;

  return (
    <Section title="Stored Materials">
      {stored_materials.length === 0 ? (
