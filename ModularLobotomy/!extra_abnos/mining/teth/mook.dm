...
/mob/living/simple_animal/hostile/abnormality/mining/mook
	name = "wanderer"
	...
	move_to_delay = 2
	damage_coeff = list(RED_DAMAGE = 0.6, WHITE_DAMAGE = 1, BLACK_DAMAGE = 1.2, PALE_DAMAGE = 2)
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/CanAllowThrough(atom/movable/O)
	. = ..()
	if(istype(O, /mob/living/simple_animal/hostile/abnormality/mining/mook))
		var/mob/living/simple_animal/hostile/abnormality/mining/mook/M = O
		if(M.attack_state == MOOK_ATTACK_ACTIVE && M.throwing)
			return TRUE

/mob/living/simple_animal/hostile/abnormality/mining/mook/death()
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/AttackingTarget()
	if(isliving(target))
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/Goto()
	if(attack_state != MOOK_ATTACK_NEUTRAL)
		return
	return ..()

/mob/living/simple_animal/hostile/abnormality/mining/mook/Move()
	if(attack_state == MOOK_ATTACK_WARMUP || attack_state == MOOK_ATTACK_RECOVERY)
		return
	return ..()

/mob/living/simple_animal/hostile/abnormality/mining/mook/proc/WarmupAttack(forced_slash_combo = FALSE)
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/proc/SlashCombo()
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/proc/SlashAttack()
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/proc/LeapAttack()
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/proc/AttackRecovery()
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/proc/ResetNeutral()
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	. = ..()
	...
	for(var/A in get_turf(src))
		...
		if(isliving(A))
			...
			if(!struck_target_leap && CanAttack(ML))//Check if some joker is attempting to use rest to evade us
				...
				update_icons()
				continue
			if(istype(ML, /mob/living/simple_animal/hostile/abnormality/mining/mook) && !mook_under_us)//If we land on the same tile as another mook, spread out so we don't stack our sprite on the same tile
				var/mob/living/simple_animal/hostile/abnormality/mining/mook/M = ML
				if(!M.stat)
					mook_under_us = TRUE
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/handle_automated_action()
	if(attack_state)
		return
	return ..()

/mob/living/simple_animal/hostile/abnormality/mining/mook/OpenFire()
	if(isliving(target))
		var/mob/living/L = target
		if(L.incapacitated())
			return
...
/mob/living/simple_animal/hostile/abnormality/mining/mook/update_icons()
	. = ..()
	if(!stat)
		switch(attack_state)
