	...
	switch(serum)
		if("R") // Thanks helper
			...
			for(var/mob/living/L in mobs_to_hit)
				...
				L.deal_damage(justicemod*60, RED_DAMAGE, user)
			dash_charges--
			if(dash_charges == 0)
				special_attack = FALSE
...
/obj/item/ego_weapon/the_claw/proc/SerumW(mob/living/user, mob/living/target) // It wasn't nice meeting you, farewell.
	...
	for(var/i = 1 to 5) // Thanks egor
		...
		for(var/mob/living/AA in range(1, target_turf))
			...
			AA.deal_damage(justicemod*50, BLACK_DAMAGE, user)
	...
	for(var/mob/living/AA in range(2, target))
		...
		AA.deal_damage(justicemod*60, BLACK_DAMAGE, user) // Shouldn't have gotten close.
		new /obj/effect/temp_visual/cleave(get_turf(AA))
	...
	target.deal_damage(justicemod*150, BLACK_DAMAGE, user) // 150 so that it can scale form justice to about 300
...
/obj/item/ego_weapon/the_claw/proc/TriSerum(mob/living/user) // from PT, which was from Blue reverb
	...
	for(var/mob/living/L in targets)
		...
		L.deal_damage(justicemod*60, PALE_DAMAGE, user, overrides = list(blocked = L.run_armor_check(null, BLACK_DAMAGE), spread_damage = TRUE))
