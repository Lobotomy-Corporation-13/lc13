...
/obj/item/ego_weapon/city/thumb_east
	...
	"Spending ammo with this weapon generates <b>heat</b>. Heat increases the base damage of the weapon when <b>not</b> using combo attacks. It decays on each hit and is <b>reset on reloading or unloading</b>."
...
/obj/item/ego_weapon/city/thumb_east/examine(mob/user)
	...
	if(overheat > 0)
		...
		if(apply_overheat_burn)
			. += span_danger("This weapon will [next_hit_should_apply["burn"] > 0 ? "apply [next_hit_should_apply["burn"]] burn stacks" : "not apply burn stacks"] on a non-combo hit due to the heat generated by spent rounds.")
...
/obj/item/ego_weapon/city/thumb_east/proc/ReturnToNormal(mob/user)
	...
	if(apply_overheat_burn)
		...
		if(augment_burn_to_apply >= 1)
			next_hit_should_apply["burn"] = augment_burn_to_apply
