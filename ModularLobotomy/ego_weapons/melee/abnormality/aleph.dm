...
/obj/item/ego_weapon/goldrush/attack(mob/living/target, mob/living/user)
	...
	if(do_after(user, 4, target))
		...
		target.deal_damage(goldrush_damage, RED_DAMAGE, user)		//MASSIVE fuckoff punch
		...
		for(var/mob/living/L in T)
			if(special_checks_faction && user.faction_check_mob(L))
				if(ishuman(L))
					...
					continue
			L.deal_damage(modified_damage, BLACK_DAMAGE, user)
...
/obj/item/ego_weapon/space/attack(mob/living/target, mob/living/user)
	...
	target.deal_damage(damage, BLACK_DAMAGE, user)
	...
	if(do_after(user, 5, src, IGNORE_USER_LOC_CHANGE))
		...
		for(var/mob/living/L in range(3, user))
			...
			L.deal_damage(aoe, WHITE_DAMAGE, user)
	icon_state = "space"
	update_icon_state()
	canaoe = FALSE
	...
	if(do_after(user, 5))
		for(var/turf/open/T in range(target_turf, 1))
			...
			for(var/mob/living/L in T.contents)
				L.deal_damage(modified_damage, WHITE_DAMAGE, user)
				if((L.stat < DEAD) && !(L.status_flags & GODMODE))
					damage_dealt += modified_damage
...
/obj/item/ego_weapon/oberon/attack(mob/living/target, mob/living/carbon/human/user)
	...
	switch(form)
		if("sword")
			var/red = force
			red*=justicemod
			target.deal_damage(red * force_multiplier, RED_DAMAGE, user)
		...
		if("hammer")
			for(var/mob/living/L in view(2, target))
				...
				if(user.faction_check_mob(L))
					continue
				L.deal_damage(aoe * force_multiplier, BLACK_DAMAGE, user)
				new /obj/effect/temp_visual/small_smoke/halfsecond(get_turf(L))
				if(!ishuman(L))
	...
	for(var/i = 0; i < 3; i++)
		for(var/turf/T in view(3, user))
			...
			for(var/mob/living/L in T)
				if(user.faction_check_mob(L))
					continue
				L.deal_damage(smash_damage * force_multiplier, BLACK_DAMAGE, user)
...
/obj/item/ego_weapon/shield/gasharpoon/attack(mob/living/M, mob/living/user)
	...
	for(var/mob/living/L in view(1, M))
		...
		if(L == user || ishuman(L))
			continue
		L.deal_damage(aoe, PALE_DAMAGE, user)
		new /obj/effect/temp_visual/small_smoke/halfsecond(get_turf(L))
